...
export default function Page() {
  ...
  // === AUTO-OBLICZANIE DLA FORMULARZA "НОВАЯ СДЕЛКА" Z RĘCZNĄ PROWIZJĄ ===
  useEffect(() => {
    ...
    // USDT → FIAT
    if (
      amountInCurrency === "USDT" &&
      amountOutCurrency !== "USDT"
    ) {
      ...
      if (hasCustomPercent) {
        ...
        const { clientFiat: c, feeFiat: f } = calcCryptoToFiat(
          amountInNum,
          rateNum
        );
        clientFiat = c;
    ...
    // FIAT → USDT
      ...
      if (hasCustomPercent) {
        ...
        const { clientUsdt: c, feeUsdt: f } = calcFiatToCrypto(
          amountInNum,
          rateNum
        );
        clientUsdt = c;
        ...
        <div className="w-full max-w-sm rounded-3xl border border-slate-800 bg-slate-950/80 p-6 shadow-xl">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="mt-4 text-[10px] text-slate-500">
              ...
              <p>
                worker:{" "}
                <span className="text-emerald-300">worker / worker123</span>
              </p>
            </div>
          </div>
        </div>
